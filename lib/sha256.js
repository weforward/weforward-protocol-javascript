 ...
 */
 function SHA256() {
	...
	function Utf8Encode(string) {
		...
		let utftext = [];
		let leadSurrogates = 0;
		for (let n = 0; n < string.length; n++) {
			let c = string.charCodeAt(n);
			if (c < 128) { // 000000 - 00007F
				utftext.push(String.fromCharCode(c));
			} else if ((c > 127) && (c < 2048)) { // 000080 - 0007FF
				utftext.push(String.fromCharCode((c >> 6) | 192));
				utftext.push(String.fromCharCode((c & 63) | 128));
			} else if((c >= 2048 && c < 55295) || (c >= 57344 && c <= 65535)) { // 000800 - 00D7FF ; 00E000 - 00FFFF
				utftext.push(String.fromCharCode((c >> 12) | 224));
				utftext.push(String.fromCharCode(((c >> 6) & 63) | 128));
				utftext.push(String.fromCharCode((c & 63) | 128));
			} else { // 010000 - 10FFFF 辅助平面字符，由utf-16代理对组成
				if(0 == leadSurrogates){
					leadSurrogates = c;
				} else {
					let trailSurrogates = c;
					// 先解码utf-16
					c = (leadSurrogates & 0x3ff) << 10;
					c |= (trailSurrogates & 0x3ff);
					c += 0x10000;
					// 再编码utf-8
					utftext.push(String.fromCharCode((c >> 18) | 0xf0));
					utftext.push(String.fromCharCode(((c >> 12) & 0x3f) | 0x80));
					utftext.push(String.fromCharCode(((c >> 6) & 0x3f) | 0x80));
					utftext.push(String.fromCharCode((c & 0x3f) | 0x80));
