...
function refreshAccess() {
	...
	let url = GLOBALCONFIG['refreshAccessUrl'] || 'user?method=refresh_access';
...
WeforwradProtocol.prototype = {
	...
	login(username, password) {
		return new Promise((resove, reject) => {
			let url = GLOBALCONFIG['loginUrl'] || 'user?method=login';
			noAccessPost(url, {
				userName: username,
				password: password,
			}).then(data => {
	...
	su(username, password, suloginname) {
		return new Promise((resove, reject) => {
			...
			noAccessPost(url, {
				...
				let suurl = GLOBALCONFIG['suUrl'] || 'user?method=su';
	...
	logout() {
		let promise = new Promise((resove, reject) => {
			let url = GLOBALCONFIG['logoutUrl'] || 'user?method=logout';
			post(url).finally(() => {
				removeLoginInfo();
				resove();
			})
