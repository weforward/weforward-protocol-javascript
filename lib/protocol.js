...
/**
 * 服务数据请求接口（如果包含图片文件，一律转为base64字串）
 * @param {String} url 由域名（如果配置了baseUrl，域名可以省略）+服务名+'?'+参数键值对 例如：'http:192.168.0.10:6562/ias?method=accountinfo'
...
WeforwradProtocol.prototype = {
	...
	/**
	 * 设置数据请求异常（非业务异常，例如：网络链接错误或者，response的status为500,502,503,504之类的）时,单个请求可重试的次数
	...
	upload(url, params, file, config) {
		...
		return new Promise((resoved, reject) => {
			...
			post(url, params, _config).then(data => {
				//处理返回的数据
				let resDataMode = _config.wfconfig.resDataMode === undefined ? -1 : _config.wfconfig.resDataMode;
				if (resDataMode == -1) {
					resDataMode = defalutResDataMode;
				}
				if (resDataMode == 1) {
